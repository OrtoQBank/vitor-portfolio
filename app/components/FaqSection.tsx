    ...
    >
      <div className="min-h-screen w-full flex items-center px-8 md:px-16 lg:px-24 py-12 overflow-hidden">
        <div className="container mx-auto max-w-5xl">
          <h2 className="text-5xl md:text-6xl lg:text-7xl font-bold text-foreground tracking-tighter leading-[0.9] mb-12">
            ...
            {faqs.map((faq, index) => (
              <div key={index} className="group border-b border-border">
                <div className="flex items-center justify-between py-6 cursor-default transition-all duration-300 group-hover:pl-2">
                  <span className="text-lg md:text-xl lg:text-2xl font-bold text-foreground group-hover:text-primary transition-colors duration-300">{faq.q}</span>
                ...
                <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-all duration-500 ease-out">
                  <div className="overflow-hidden">
                    <p className="text-base md:text-lg text-foreground-muted leading-relaxed font-medium pb-6 pr-12 pl-2">{faq.a}</p>
                  </div>
                </div>
              </div>
            ))}
